{
  programs.fish = {
    ...
    shellAliases = {
      ...
      lt = "eza --tree";
      cat = "bat";
      update = "cd ~/nixos-config && nix --extra-experimental-features 'nix-command flakes' flake update && git add flake.lock && git commit -m 'update packages' && git push && sudo nixos-rebuild switch --flake ~/nixos-config#(hostname)";
